...
To get a full list of differences run \Rcmd{R CMD check HSAUR} on the source package. All issues marked
with \textbf{R1} have been silently fixed in the first reprint.
\section*{Preface}
