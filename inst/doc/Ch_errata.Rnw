...
Typo at page 31, code line 4: use argument \Rcmd{varwidth = TRUE}, not \Rcmd{var.width = TRUE}.
\section{Conditional Inference}
\begin{itemize}
\item The names of the test statistics in the output have been changed from \Rcmd{T} to \Rcmd{Z}
or \Rcmd{chi-squared} throughout the chapter.
\item Reference \cite{HSAUR:Hothorn:2006:AmStat} updated
\end{itemize}
\section{Analysis of Variance}
--nothing known--
\section{Multiple Linear Regression}
Page 83: both \Rcmd{fitted} and \Rcmd{predict} can be used to compute fitted values, the later on
can be applied to new unseen data as well.
\section{Logistic Regression and Generalised Linear Models}
Function \Rcmd{myplot} (page 100): the \Rcmd{vfont} argument in \Rcmd{text} has been changed
to \Rcmd{family = "HersheySerif"} (the resulting plots remain the same).
...
\item Reference \cite{HSAUR:Hothorn:2006:JCGS} updated
\end{itemize}
\section{Survival Analysis}
The name of the test statistic in the output of \Rcmd{surv\_test} has been changed to
\Rcmd{chi-squared}.
...
update to \Rpackage{mclust} version 3.0-0 (new plot method)
\section*{Thanks}
We would like to thank the following people for pointing out errors, typos etc and for
making suggestions for improvements:
\begin{itemize}
\item Tobias Verbeke
\item Brian D. Ripley
\end{itemize}
\bibliographystyle{../LaTeX/refstyle}
\bibliography{../LaTeX/HSAUR}
\end{document}
