...
dangerous to rely on asymptotic tests. As shown in Chapter~\ref{CI},
...
\Rcmd{surv\_test} from package \Rpackage{coin} \citep{PKG:coin} can be used
...
models in Chapters~\ref{ANOVA}, \ref{MLR} or \ref{GLM} with the exception
...
in Chapter~\ref{MLR} for linear regression model are not available,
...
responses in Chapter~\ref{RP} are applicable to censored responses in
