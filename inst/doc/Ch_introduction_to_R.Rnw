...
\index{Base system|)}
%%% this is `pixeled LaTeX', fix if possible %'
\vspace*{\parsep}
\noindent
\begin{minipage}[c]{0.3\textwidth}
%%% <FIXME> transparent background
%%\begin{figure}[h!]
%%\pagecolor{white}
\begin{center}
...
%%% </FIXME>
\end{minipage}
\begin{minipage}[c]{0.688\textwidth}
...
is required (for example in Chapter~\ref{ALDII}),
...
such as smooth density estimates will be considered in Chapter~\ref{DE}.)
\begin{figure}
\begin{center}
...
the linear model function as explained in Chapter~\ref{MLR}). The
\Rcmd{plot} generic function implements a \Rclass{formula} method as well. Because
